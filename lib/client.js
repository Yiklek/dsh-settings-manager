...
function createManagerSection(env) {
  ...
  function ManagerSection() {
    ...
    function renderRow(row, index) {
      ...
      const className = ["dsm-row", dragging ? "dsm-row-dragging" : "", isTarget ? "dsm-drop-target" : ""].filter(Boolean).join(" ");
    ...
    const body = rows.length === 0 ? import_react.default.createElement("p", { style: styles.empty }, t("noSections")) : import_react.default.createElement("ul", { style: styles.list }, rows.map((row, index) => renderRow(row, index)));
